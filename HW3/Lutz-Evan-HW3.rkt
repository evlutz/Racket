...
;Merchandise -> BOOLEAN
;Takes in an define struct and determines if the item is a Merchandise
(define (merchandises? merch)
  (Merchandise? merch))
...
(define (halloween-sale lom discount)
  (cond [(empty? lom) 0]
        [(string=? "costume" (Merchandise-kind(first lom)))
         (if (string=? (Merchandise-kind(first lom)) "costume")
                         (+ (* (Merchandise-quantity(first lom))(* (Merchandise-price(first lom)) discount)) (halloween-sale(rest lom) discount))
